...
def compute_pacing_metrics(cuts: list[float], duration: float) -> dict:
    ...
    Returns:
        다음 키를 포함하는 dict:
            cut_count (int): 컷 수
            cut_count_range (list[int]): [cut_count, cut_count]
    ...
    if avg_shot_len_sec < _FAST_MONTAGE_MAX:
        rhythm_mode = "fast_montage"
    elif avg_shot_len_sec > _SLOW_HOLD_MIN:
    ...
    return {
        "cut_count": cut_count,
        "cut_count_range": [cut_count, cut_count],
